...
class Focus:
        ...
        for line in metricsRaw.prettify().split("\n"):
            if 'wifi_network_quality{mode' in line:
                ...
                if f'ssid="{self.pvt_name}"' in line:
                  ...
                  self.pvt_interface_data_name = pvt_interface_data.split("{")[1].split(",")[1].split("=")[1][1:][:-1]
                  self.log.info("Private interface name find: " + pvt_interface_data_name)
                  ...
                  for line2 in metricsRaw.prettify().split("\n"):
                      if 'wifi_stations{ifname="' + self.pvt_interface_data_name in line2:
                          self.pvt_interface_stations_num = line2.split(" ")[1]
                  ...
                  if "0" in self.pvt_interface_stations_num:
                      self.log.info("0 stations active, adding a occurrence and checking for 10 consecutive 0 occurrences.")
                      self.addZeroOccurrence()
                      self.checkZeroOccurrences(self.pvt_interface_data_name)
                  else:
                      self.resetList()
                ...
                if f'ssid="{self.pub_name}"' in line:
                  ...
                  self.pub_interface_data_name = pub_interface_data.split("{")[1].split(",")[1].split("=")[1][1:][:-1]
                  self.log.info(f"Pub interface data found: {self.pub_interface_data_name}!")

        if pvt_interface_data is '' or pub_interface_data is '':
            #self.run = False
            self.log.error("No pvt or pub interface data was find on metric response! Trying again")
            #os._exit(1)
    ...
    def addZeroOccurrence(self):
        self.log.info("Zero wifi_stations detected, registring the occurrence...")
    ...
    def callElasticyMechanism(self):
        ...
        self.requestChanges(self.pub_interface_data_name, self.pvt_interface_data_name)
    ...
    def requestChanges(self, pub_interface_data_name, pvt_interface_data_name):
        ...
        self.log.info("Making HTTP Post request to http://10.7.229.85:8089/necos/wscagent/ssid/update")
